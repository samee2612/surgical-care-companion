...
try:
    ...
    print('[INFO] This is normal on first run')
" 2>nul
...
REM Use PowerShell for better JSON handling
powershell -Command "try { $response = Invoke-RestMethod -Uri 'http://localhost:4040/api/tunnels' -TimeoutSec 10; $httpsUrl = ($response.tunnels | Where-Object { $_.proto -eq 'https' }).public_url; if ($httpsUrl) { Write-Host '[SUCCESS] ngrok URL:' $httpsUrl; Write-Host '[INFO] Configure Twilio webhook URL to:'; Write-Host '       ' $httpsUrl'/api/v1/twilio/voice-webhook/' } else { Write-Host '[WARNING] No HTTPS tunnel found' } } catch { Write-Host '[WARNING] Could not get ngrok URL - check http://localhost:4040' }"
